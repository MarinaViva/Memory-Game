...
// Funktion för att uppdatera texten som visar vems tur det är och vem som har vunnit
function updatePlayerTurnText() {
  ...
  if (document.querySelectorAll('.card:not(.matched)').length === 0) {
    // Alla kort har matchat, spelet är över
    const winnerMessageElement = document.getElementById('winner-message');

    if (player1Score > player2Score) {
      winnerMessageElement.textContent = `Grattis! ${player1Name} vann med ${player1Score} poäng.`;
    } else if (player2Score > player1Score) {
      winnerMessageElement.textContent = `Grattis! ${player2Name} vann med ${player2Score} poäng.`;
    } else {
      winnerMessageElement.textContent = 'Det blev oavgjort!';
    }

    // Dölj meddelandet om vems tur det är
    currentPlayerTurnElement.style.display = 'none';
  } else {
    // Visa vems tur det är att välja ett kort
    currentPlayerTurnElement.textContent = `Det är ${currentPlayerName}s tur att välja ett kort.`;

    // Visa meddelandet om vems tur det är och dölj vinnarmeddelandet
...
// Updated initializeGame function, used to start the game by creating and placing cards on the game board
...
// Compare players' scores and display a message about the winner
function increasePlayerScore() {
  ...
  if (player1Score > player2Score) {
    winnerMessageElement.textContent = `Grattis! Spelare 1 vann med ${player1Score} poäng.`;
  } else if (player2Score > player1Score) {
    winnerMessageElement.textContent = `Grattis! Spelare 2 vann med ${player2Score} poäng.`;
  } else {
